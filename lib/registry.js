    ...
    if (this.integrity) {
      ...
      if (dist.attestations) {
        if (this.opts.verifyAttestations) {
          ...
          const keySelector = (keyid) => {
            const publicKey = attestationRegistryKeys.find(key => key.keyid === keyid)
            return publicKey ? publicKey.pemkey : undefined
          }
          ...
          let verifyBundle
          try {
            verifyBundle = await sigstore.createVerifier(verifyOptions)
          } catch (e) {
            throw Object.assign(new Error(
              `${mani._id} failed to create attestation verifier: ${e.message}`
            ), {
